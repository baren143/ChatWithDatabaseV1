...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect to the chat app after successful login
      router.push('/app');
    } catch (err) {
      ...
      <header className="border-b border-white/5 bg-[#07090e]/40 backdrop-blur-md sticky top-0 z-50 relative z-20">
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center relative z-20">
          ...
          <div className="flex items-center gap-4">
            <Link href="/auth/login" className="text-sm font-semibold text-white bg-white/5 border border-white/10 px-3 py-2 rounded-xl hover:bg-white/10 transition-colors">
              Sign In
            </Link>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4 relative z-10">
        ...
        <div className="text-center mb-8 max-w-md">
          ...
          <p className="text-sm text-gray-400 mt-3 leading-relaxed">
            Sign in to access your documents and AI-powered chat workspace.
          </p>
        </div>
